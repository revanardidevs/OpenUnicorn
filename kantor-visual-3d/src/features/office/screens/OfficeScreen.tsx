...
export function OfficeScreen({
  ...
  useEffect(() => {
    ...
    if (selectedAdapterType === "custom" || selectedAdapterType === "openclaw") {
      const preset = activeFloor.kind === "lobby" ? "lobby" : "office";
      const furniture = materializeDefaults(preset);
      const desks = furniture.filter((item) => item.type === "desk_cubicle");
      const mapping: Record<string, string> = {};
      if (desks[0]) mapping[desks[0]._uid] = "product_manager";
      if (desks[1]) mapping[desks[1]._uid] = "fullstack_developer";
      if (desks[2]) mapping[desks[2]._uid] = "qa_engineer";
      setDeskAssignmentByDeskUid(mapping);
      return;
    }
    void (async () => {
      try {
  ...
  }, [gatewayUrl, selectedAdapterType, activeFloor.kind, loadStudioSettings]);
  ...
  const loadAgents = useCallback(async (options?: {
    ...
    if (selectedAdapterType === "custom" || selectedAdapterType === "openclaw") {
      const customAgents = [
        {
          ...
          runtimeName: "Product Manager",
          identityName: "Product Manager",
          sessionDisplayName: "Product Manager",
          role: "Product Manager",
          sessionKey: "product_manager",
          avatarSeed: "product_manager",
          avatarProfile: createDefaultAgentAvatarProfile("product_manager"),
          model: "crewai/pm",
          thinkingLevel: "high",
          toolCallingEnabled: false,
        ...
        },
        {
          ...
          runtimeName: "Full-Stack Developer",
          identityName: "Full-Stack Developer",
          sessionDisplayName: "Full-Stack Developer",
          role: "Full-Stack Developer",
          sessionKey: "fullstack_developer",
          avatarSeed: "fullstack_developer",
          avatarProfile: createDefaultAgentAvatarProfile("fullstack_developer"),
          model: "crewai/developer",
          thinkingLevel: "high",
          toolCallingEnabled: false,
          showThinkingTraces: true,
        },
        {
          agentId: "qa_engineer",
          name: "QA Engineer",
          runtimeName: "QA Engineer",
          identityName: "QA Engineer",
          sessionDisplayName: "QA Engineer",
          role: "QA Engineer",
          sessionKey: "qa_engineer",
          avatarSeed: "qa_engineer",
          avatarProfile: createDefaultAgentAvatarProfile("qa_engineer"),
          model: "crewai/qa",
          thinkingLevel: "high",
          toolCallingEnabled: false,
          showThinkingTraces: true,
      ...
      hydrateAgents(customAgents, "product_manager");
      setAgentsLoaded(true);
      return;
    }
  ...
  useEffect(() => {
    ...
    const unsubscribeEvent = client.onEvent((event) => {
      ...
      if (event.event === "agent.status") {
        const payload = event.payload as { agent: string; status: string };
        if (payload && payload.agent) {
          const isWorking = payload.status === "coding" || payload.status === "testing";
          dispatch({
            type: "updateAgent",
